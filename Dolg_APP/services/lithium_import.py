...
def parse_lpr(text: str) -> dict[str, Any]:
    ...
    layers = [
        ...
        for layer in (project.findall('library/layers/layer') or project.findall('layers/layer'))
    ]
    ...
    if pages_el is not None:
        for page in pages_el.findall('page'):
            ...
            for fragment in page.findall('fragments/fragment'):
                rpins = [
                    {'id': rp.get('id', ''), 'name': rp.get('name', '')} for rp in fragment.findall('rpin')
                ]
                wires_count = len(fragment.findall('wire'))
                junctions_count = len(fragment.findall('junction'))
                labels = [{'x': lbl.get('x', ''), 'y': lbl.get('y', '')} for lbl in fragment.findall('label')]
                nets.append(
                    {
...
def parse_lbo(text: str) -> dict[str, Any]:
    ...
    layers = [
        {'id': layer.get('id', ''), 'name': layer.get('name', '')} for layer in pcb.findall('layers/layer')
    ]
