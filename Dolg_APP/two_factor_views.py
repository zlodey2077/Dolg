...
def two_factor_setup(request):
    ...
    if request.method == 'POST':
        ...
        if two_factor.confirm_totp_device(device, token):
            ...
            backup_codes = list(
                device.user.staticdevice_set.filter(name='backup')
                .first()
        ...
        messages.error(
            request, 'Неверный код. Попробуйте ещё раз — код в Authenticator меняется каждые 30 сек.'
        )
...
def two_factor_verify(request):
    ...
    if user.is_verified() or not two_factor.user_has_confirmed_totp(user):
        return redirect(request.session.pop('_dolg_pending_2fa_next', None) or reverse('accounts:profile'))
    ...
    if request.method == 'POST':
        ...
        for device in StaticDevice.objects.filter(user=user, confirmed=True):
            if device.verify_token(token):
                ...
                messages.warning(
                    request, f'✓ Использован backup-код. Осталось {remaining} — рекомендуем перегенерировать.'
                )
                next_url = request.session.pop('_dolg_pending_2fa_next', None) or reverse('accounts:profile')
                return redirect(next_url)
...
def two_factor_backup_codes_regenerate(request):
    ...
    messages.warning(request, '🔄 Старые backup-коды отозваны. Запишите новые — они показаны ниже.')
    return render(
        request,
        'auth/2fa_backup_codes.html',
        {
