...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--reb', action='store_true', help='Индексировать только РЭБ-категории (8 категорий)'
        )
    ...
    def handle(self, *args, **opts):
        ...
        if not is_semantic_available():
            self.stderr.write(
                self.style.ERROR('scikit-learn не установлен. Установите: pip install scikit-learn')
            )
            return

        if is_index_stale():
            self.stdout.write(
                self.style.WARNING('⚠ Индекс помечен как stale (Product был изменён) — нужен rebuild.')
            )
