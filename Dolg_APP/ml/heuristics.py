...
def count_components_by_type(scheme_data: dict) -> dict:
    ...
    return dict(Counter(c.get('type', 'unknown') for c in (scheme_data.get('components') or [])))
...
def find_cycles_without_active(graph: dict, scheme_data: dict) -> list:
    ...
    has_active = any(c.get('type') in ACTIVE_TYPES for c in (scheme_data.get('components') or []))
...
_USE_CASE_DESCRIPTIONS = {
    ...
    'unknown': 'Нестандартная топология. Возможно, пользовательская сборка для специфической задачи.',
}
...
def generate_scheme_description(scheme_data: dict) -> dict:
    ...
    significant_types = {k: v for k, v in types.items() if k not in INFRASTRUCTURE_TYPES}
    ...
    summary = (
        f'Схема содержит {components_str}. '
...
_NEXT_COMPONENT_RULES = [
    ...
    (
        ...
        ['ground'],
        'Каждой схеме с источником нужна земля (общий ноль).',
        0.95,
    ),
    (lambda t: sum(t.values()) == 0, ['battery'], 'Начните с источника питания.', 0.9),
    (
        lambda t: t.get('battery', 0) > 0 and t.get('resistor', 0) == 0 and t.get('led', 0) > 0,
        ['resistor'],
        'LED нужен токоограничительный резистор перед ним.',
        0.9,
    ),
    (
        lambda t: t.get('resistor', 0) > 0 and t.get('capacitor', 0) == 0 and t.get('battery', 0) > 0,
        ['capacitor'],
        'Добавьте конденсатор — получите RC-фильтр или развязку.',
        0.6,
    ),
    (
        lambda t: t.get('npn', 0) > 0 and t.get('resistor', 0) < 2,
        ['resistor'],
        'Транзисторный каскад требует делитель базы (2 резистора).',
        0.85,
    ),
    (
        lambda t: t.get('diode', 0) >= 1 and t.get('capacitor', 0) == 0,
        ['capacitor'],
        'После диода — сглаживающий конденсатор для DC-выхода.',
        0.7,
    ),
]
