...
def group_parameters(params):
    ...
    if remaining:
        extras = [(PARAM_LABELS.get(k, k.replace('_', ' ').capitalize()), v) for k, v in remaining.items()]
...
CARD_PARAM_LABELS = {
    ...
    'camera': 'Камера',
    'battery': 'Батарея',
    'battery_earbuds': 'Бат. (наушн.)',
    'battery_case': 'Бат. (кейс)',
    'charging': 'Заряд.',
    'os': 'ОС',
    'weight': 'Масса',
    'chassis': 'Корпус',
...
CARD_PARAM_FILTER_MAP = {
    ...
    'vram': 'capacity',
    'battery': 'capacity',
    'battery_earbuds': 'capacity',
    'battery_case': 'capacity',
    'cache_l3': 'capacity',
    'screen_size': 'display',
    'resolution': 'display',
    'panel': 'display',
    ...
    'inputs': 'connectivity',
    'power_conn': 'connectivity',
    'm2_slots': 'connectivity',
    'pcie': 'connectivity',
    'ram_slots': 'connectivity',
    ...
    'operating_temp': 'temperature_range',
    'max_temp': 'temperature_range',
    'temp_max': 'temperature_range',
    'min_temp': 'temperature_range',
    'melting_point': 'temperature_range',
    'mode': 'mode',
    'signal': 'mode',
    'safety': 'safety',
}
...
def chip_filter_name(key):
    ...
    if key and key not in CARD_PARAM_EXCLUDE and not (isinstance(key, str) and key.startswith('_')):
