...
SMD_HINTS = (
    'SMD', 'SMT', 'SOIC', 'SOT', 'SOD', 'SMA', 'SMB', 'QFN', 'TSSOP',
    'MSOP', '0603', '0805', '1206', '1210', '2512', 'CD43', 'USB-C',
    'MICRO-B', 'SOT-223',
)

THT_HINTS = (
    'THT', 'DIP', 'PDIP', 'TO-92', 'TO-18', 'TO-220', 'DO-35', 'DO-41',
    'DO-201', 'AXIAL', 'RADIAL', 'HEADER', 'DUPONT', 'JST-XH', 'DB9',
    'SCREW', 'TERMINAL', 'RELAY', 'TOR',
)

FAMILY_DATASHEETS = {
    ('resistors', 'yageo'): 'https://www.yageo.com/upload/media/product/productsearch/datasheet/rchip/PYu-RC_Group_51_RoHS_L_12.pdf',
    ...
    ('capacitors', 'kemet'): 'https://www.kemet.com/content/dam/kemet/lightning/migration-pdfs/AutoECML/TS0004.pdf',
    ...
    ('connectors', 'phoenix_terminal'): 'https://www.phoenixcontact.com/en-pc/products/pcb-terminal-block-mkds-1-2-35-1725656',
    ...
    ('connectors', 'dc_jack'): 'https://www.cuidevices.com/catalog/interconnect/connectors/dc-power-connectors/pj-102ah',
...
def _infer_category_parameters(product, pn: str, params: dict[str, Any], result: NormalizationResult, *, force: bool = False) -> None:
    category = getattr(product.category, 'slug', '')
    token = pn.upper()
...
def _infer_inductor_params(pn: str, params: dict[str, Any], result: NormalizationResult) -> None:
    ...
    if pn.startswith('CD43'):
        current = '2 А' if '1UH' in pn else ('1.5 А' if '10UH' in pn else ('800 мА' if '100UH' in pn else '400 мА'))
        dcr = '50 мОм' if '1UH' in pn else ('120 мОм' if '10UH' in pn else ('600 мОм' if '100UH' in pn else '900 мОм'))
        srf = '120 МГц' if '1UH' in pn else ('35 МГц' if '10UH' in pn else ('8 МГц' if '100UH' in pn else '5 МГц'))
        _merge_params(params, result, {'type': 'SMD power inductor', 'current': current, 'dcr': dcr, 'srf': srf})
    elif pn.startswith('AXIAL'):
        _merge_params(params, result, {'type': 'axial inductor', 'current': '100 мА', 'dcr': 'до 3 Ом', 'srf': 'зависит от номинала'})
    elif pn.startswith('TOR'):
        _merge_params(params, result, {'type': 'toroidal inductor', 'current': '3 А', 'dcr': 'низкое', 'srf': 'power choke'})
...
def _infer_relay_params(pn: str, params: dict[str, Any], result: NormalizationResult) -> None:
    ...
    if pn.startswith('SSR'):
        _merge_params(params, result, {'type': 'solid state relay', 'coil_voltage': '3...32 В DC', 'current': '25 А', 'voltage': '24...380 В AC'})
    elif pn.startswith('SRD') or pn.startswith('JQX') or pn.startswith('G5LE'):
        current = '16 А' if pn.startswith('JQX') else '10 А'
        _merge_params(params, result, {'type': 'power relay', 'coil_voltage': voltage, 'contact_rating': f'{current} / 250 В AC', 'current': current})
    elif pn.startswith('HL-'):
        _merge_params(params, result, {'type': 'signal relay', 'coil_voltage': voltage, 'contact_rating': '1 А / 30 В DC', 'current': '1 А'})
