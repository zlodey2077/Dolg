...
4. Авто-протокол `.md`/PDF.
   - Готово: `.md`-генератор умеет проектный режим через `project_id` и собирает схему, BOM/readiness, историю симуляций, измерения, review findings и источники без сохранения лишнего `ProjectReview`; в симуляторе есть preview/download модалка и PDF-скачивание.
   - Осталось: графики/осциллограммы.
