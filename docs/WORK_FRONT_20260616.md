...
- Docker проверен после перезагрузки 2026-06-18: VS Code stack health-check проходит по расширениям/Python/frontend, но Docker Desktop остаётся в `Status=starting`; `docker info/version` доходят до pipe `dockerDesktopLinuxEngine`, но получают `500 Internal Server Error`. WSL 2 установлен (`2.7.8.0`), однако `wsl --list --all --verbose` не видит дистрибутивы. Admin repair `logs/windows-stack-repair-20260618-080237.log` поднял `vmcompute`, `LxssManager`, `com.docker.service` и снова требует restart до надёжного Docker-теста; `UseContainerdSnapshotter` временно выключен с backup `settings-store.json.bak-20260618-075626`.
...
4. Авто-протокол `.md`/PDF.
   - Готово: `.md`-генератор умеет проектный режим через `project_id` и собирает схему, BOM/readiness, историю симуляций, summary осциллограмм/артефактов, измерения, review findings и источники без сохранения лишнего `ProjectReview`; в симуляторе есть preview/download модалка и PDF-скачивание.
   - Осталось: полноценные PNG/SVG-графики для PDF/приложений.
...
0. Клиентский интерфейс - отдельный большой проход позже.
   - Не смешивать с мелкими серверными задачами: нужна цельная ревизия UX, навигации, состояний, ошибок, адаптива и визуальной системы.
   - Больная зона проекта: брать как самостоятельный фронт после закрытия текущих P0/P1 технических блоков.
...
2. Провода и UX редактора.
   ...
   - Готово 2026-06-18: дедуп DRC/review "Отсутствует GND" - экспертное правило `erc.missing_ground` стало каноническим источником GND-рекомендации.
