...
4. Авто-протокол `.md`/PDF.
   - Готово: `.md`-генератор умеет проектный режим через `project_id` и собирает схему, историю симуляций, измерения и review findings без сохранения лишнего `ProjectReview`; в симуляторе есть preview/download модалка и PDF-скачивание.
   - Осталось: графики/осциллограммы и BOM/sources.
