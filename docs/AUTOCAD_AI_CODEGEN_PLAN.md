...
| Идея | Приоритет | Связь с готовым в DOLG | Честная оценка |
|---|---|---|---|
| **Обучающий тренажёр**: студент пишет ТЗ на естественном языке → система генерит чертёж/схему и **подсвечивает ошибки** («забыл допуск/слой/землю») | 🟢 P1–P2 | Прямо ложится на уже готовые `validator`/DRC/ERC + `layout_quality` + модуль `knowledge`/обучения | Самый реалистичный «вау» для защиты: генератор может быть слабым, а ценность даёт **проверка и подсветка** на готовом движке. Ошибки ловит код, не модель |
| **Автогенерация типовых проектов**: ТЗ («1-комн. 40 м², кухня-гостиная») → план стен/мебель/розетки | 🟡 P2 | Это и есть «финальный тест» плана (apartment plan) через CAD-JSON → ezdxf | Требует всего пайплайна (intent → CAD-JSON → validator → DXF). Начинать с шаблонных типовых планов, AI — поверх |
| **Динамический каталог деталей**: «винт M8×30» → параметрическая 3D-модель по ГОСТ | 🟡 P2 | Смыкается с **магазином** (товар↔модель) и **3D-вектором** (pythonOCC/CadQuery, STEP) | Параметрические крепёж/корпуса генерируются детерминированно (не нужна большая модель); ГОСТ-таблицы → геометрия. Ценно как мост каталог↔3D |
| **Реконструкция чертежа по фото** (скан/фото → DXF: линии, символы, надписи) | 🔴 P3 | Отдельный CV-домен (детекция линий/символов, OCR), к codegen не относится | Тяжёлый research (vision). Не путать с текущим codegen-вектором; делать только если будет отдельный CV-блок |
| **Мобильный инструмент для ремонта** (фото стены → разметка проводки по нормам) | 🔴 P3 / вне scope | Почти не пересекается с ядром; это отдельный продукт/стартап (мобайл + AR + CV) | Идея сильная как стартап, но за рамками диплома и текущего стека |
| **«Фабрика мемов»** (голосовой ввод → шуточный чертёж) | ⚪ skip | — | Только как пасхалка/разрядка на демо, не как функция |

**Вывод по приоритетам:** первым делать **обучающий тренажёр** — он переиспользует уже готовый проверочный контур (validator/DRC/layout guard) и даёт защите ясную «инновацию в обучении», даже если сам генератор пока шаблонный/слабый. Дальше — типовые планы и параметрический каталог деталей (мост в магазин и 3D). Фото-реконструкция и мобильный ремонт — отдельные тяжёлые домены, не смешивать с codegen-веткой.
