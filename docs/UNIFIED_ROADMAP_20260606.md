...
Обновление дорожной карты от 2026-06-05 с учётом закрытого за
...
0. 🔴 **P0 (юзер 2026-06-07) — КОМПАС-3D Python API.** Ресурс: https://vk.ru/python_macros —
   подключение API КОМПАС-3D через Python-макросы → программная генерация 3D-моделей и чертежей
   ЕСКД (мост к промышленному САПР; усиливает Mechanical-шаблоны, генератор корпусов→STL, 3D-CAD
   концепт). **Caveat:** Windows-only через COM (`win32com`/KompasAPI), серверная/десктоп-интеграция,
   не browser-side. Проверить лицензию/доступность КОМПАСа (legal-first). Заметка: [[reference-kompas-3d-python-api]].
