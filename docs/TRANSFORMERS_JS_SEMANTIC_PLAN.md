...
## Прогресс / верификация (2026-06-16)
- ✅ **Ф0 частично:** библиотека завендорена — `shop/static/ai/lib/transformers.min.js` (ESM, 888 КБ,
  jsdelivr@3). ⏳ осталось: ONNX-модель + ort-*.wasm в static для полного офлайна (сейчас фолбэк на хаб).
- ✅ **Ф1:** `shop/static/ai/dolg-embeddings.js` — модуль `ready/embed/cosine/rank/available`, синтаксис OK.
- ✅ **Feasibility доказана в Node:** `npm i @huggingface/transformers` OK; модель грузится и эмбеддит
  (dim 384). Пайплайн рабочий.
- ⚠️ **ВАЖНАЯ ПРАВКА модели:** `all-MiniLM-L6-v2` (англо) на русском корпусе путает термины
  (проверено: конденсатор~резистор 0.621 **>** конденсатор~ёмкость 0.580 — неверно). →
  **переключено на `Xenova/paraphrase-multilingual-MiniLM-L12-v2`** (384d, 50+ языков вкл. русский).
  Вес ~120 МБ (больше англо-23 МБ) → тем важнее завендорить локально (Ф0) и кэшировать.
- ⏳ **Ф2/Ф3** — впереди: corpus-эмбеддинги (Node build-скрипт) + проводка в dolg-ai-panel.
