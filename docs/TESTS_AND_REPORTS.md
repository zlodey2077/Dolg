...
- `python manage.py check_data_integrity --json` — аудит БД/данных: **0 ошибок, 0 предупреждений**; пустых, битых, Commons/curated и неподконтрольных активных изображений нет; `missing_datasheet_extracted=[]` для 43 РЭБ-товаров; `catalog.media_quality` показывает 89 проверенных изображений, `average_score=100`, `error_count=0`, `warning_count=0`, `imagehash_available=true`.
...
- Каталог-медиа: 89 товаров; 67 активных реальных фото перенесены в `products/verified/`, сомнительные старые кандидаты оставлены на generated PNG fallback. Активные карточки не ссылаются на `products/commons/`, `products/curated/` или SVG fallback; качество контролируется `shop.services.media_quality` через Pillow/ImageHash.
- Обучение и лаборатория: 4 опубликованных маршрута, 13 уроков, 29 заданий; `/knowledge/lab/` входит в demo-ready URL smoke.
