...
- P0 operational ML layer закрыт 2026-06-02: `MLJob`, `/staff/ops/`, `MLJobAdmin`, persistent progress для training/import и reset активных jobs. Следующий шаг по админке — dataset review queue, Grafana/prometheus counters и project cockpit без правок симулятора.
- Новый фронт админ-мониторинга зафиксирован в [WORKFRONT_20260603_ADMIN_MONITORING.md](WORKFRONT_20260603_ADMIN_MONITORING.md): защитить `/metrics/`, добавить runtime metrics через `psutil`, общий `ops_metrics` service-layer, расширенный `/staff/ops/`, custom `dolg_*` Prometheus metrics, Grafana dashboards, alerts и export snapshot для защиты.
- P0 AI dataset split закрыт 2026-06-02: `features.dataset_kind`, `graph_training_ready`, `training_role`, `normalize_ai_dataset_metadata`, 36 graph-ready схемных примеров и 36 text-only retrieval examples. Следующий шаг — moderation/review queue для AITrainingExample и actions “исправить metadata / исключить из graph training / promote”.
