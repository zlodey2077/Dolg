...
def update_speech_intro() -> None:
    ...
    for paragraph in doc.paragraphs:
        if "актуальной темной презентации" in paragraph.text or "актуальной тёмной презентации" in paragraph.text:
            paragraph.text = paragraph.text.replace("актуальной темной презентации", "актуальной светлой презентации")
            paragraph.text = paragraph.text.replace("актуальной тёмной презентации", "актуальной светлой презентации")
    try:
...
class CleanDeck:
    ...
    def slide(self, number: int | None = None):
        ...
        if number is not None:
            self.text_box(slide, f"{number}/12", 14.86, 8.3, 0.65, 0.3, size=12, color=self.muted, align="right")
        return slide
    ...
    def panel(self, slide, x: float, y: float, w: float, h: float, accent: RGBColor | None = None, fill: RGBColor | None = None):
    ...
    def card(
        ...
        *,
        ...
        self.text_box(slide, title, x + 0.24, y + 0.16, w - 0.38, 0.34, size=title_size, color=self.navy, bold=True)
    ...
    def metric(self, slide, x: float, y: float, w: float, h: float, value: str, label: str, accent: RGBColor) -> None:
        self.panel(slide, x, y, w, h, accent)
        self.text_box(slide, value, x + 0.06, y + 0.12, w - 0.12, 0.37, size=22, color=accent, bold=True, align="center")
        self.text_box(slide, label, x + 0.07, y + 0.57, w - 0.14, 0.38, size=12.2, color=self.muted, align="center")
    ...
    def image(self, slide, path: Path, x: float, y: float, w: float, h: float, caption: str = "") -> None:
        ...
        if caption:
            self.text_box(slide, caption, x + 0.12, y + h + 0.06, w - 0.24, 0.28, size=12, color=self.muted, align="center")

    def bullet_panel(self, slide, x: float, y: float, w: float, h: float, title: str, bullets: list[str], accent: RGBColor) -> None:
        self.panel(slide, x, y, w, h, accent)
...
def build_presentation() -> Path:
    ...
    deck.text_box(s, "Каталог -> схема -> расчет -> симуляция -> review -> заказ", 0.8, 7.25, 6.6, 0.35, size=14, color=deck.cyan, bold=True)
    ...
    deck.card(s, 0.72, 1.55, 4.35, 2.25, "Проблема", "Каталог, datasheet, CAD, SPICE, BOM и заказ часто существуют отдельно. Пользователь вручную переносит номиналы, модели и результаты.", deck.blue, body_size=14.2)
    deck.card(s, 5.42, 1.55, 4.35, 2.25, "Цель работы", "Создать веб-приложение, где компонент можно выбрать, проверить расчетом, применить в схеме, смоделировать и оформить в заказ.", deck.orange, body_size=14.2)
    deck.card(s, 10.12, 1.55, 4.95, 2.25, "Результат", "DOLG связывает каталог, CAD/SIM, лабораторию, практикум, Engineering Review, BOM и покупательский контур.", deck.green, body_size=14.2)
    metrics = [("89", "товаров"), ("43", "РЭБ"), ("21", "статья"), ("50", "материалов"), ("13", "уроков"), ("29", "заданий")]
    for i, (value, label) in enumerate(metrics):
        deck.metric(s, 1.0 + i * 2.37, 4.65, 1.65, 1.12, value, label, [deck.cyan, deck.blue, deck.green, deck.orange, deck.purple, deck.red][i])
    deck.card(s, 1.25, 6.7, 13.45, 0.96, "Формулировка эффекта", "Компонент, схема, расчет, измерение, учебное задание и заказ работают с одной моделью данных.", deck.cyan, title_size=15, body_size=14)

    # 3
    s = deck.slide()
    deck.title(s, "Анализ решений", "DOLG занимает нишу между интернет-магазином, учебной средой и CAD/SIM", 3)
    deck.table(
        s,
        0.75,
        1.45,
    ...
    deck.card(s, 1.05, 6.3, 6.75, 1.25, "Позиционирование", "Учебно-инженерный веб-сервис: пользователь проходит путь от выбора компонента до проверки и заказа без разрыва контекста.", deck.cyan, body_size=14.4)
    deck.card(s, 8.2, 6.3, 6.75, 1.25, "Развитие относительно аналогов", "Главный следующий слой - Engineering Review, который объясняет ошибки через правила, граф схемы, BOM, расчеты и измерения.", deck.green, body_size=14.4)

    # 4
    s = deck.slide()
    deck.title(s, "Целевая аудитория", "Сценарии использования системы", 4)
    deck.card(s, 0.82, 1.42, 4.25, 2.05, "Студенты", "Осваивают схемотехнику, проверяют учебные схемы, видят расчет и объяснение ошибки.", deck.blue, body_size=14.4)
    deck.card(s, 5.52, 1.42, 4.25, 2.05, "Радиолюбители", "Подбирают компоненты, собирают простые узлы, получают набор деталей для сборки.", deck.green, body_size=14.4)
    deck.card(s, 10.22, 1.42, 4.25, 2.05, "Инженеры", "Сравнивают номиналы, проверяют тепловой запас, повторяют удачные решения.", deck.orange, body_size=14.4)
    steps = [("1", "Найти компонент"), ("2", "Добавить в схему"), ("3", "Запустить расчет"), ("4", "Сравнить результат"), ("5", "Сформировать заказ")]
    for i, (num, label) in enumerate(steps):
        x = 1.1 + i * 2.88
        deck.metric(s, x, 5.15, 1.25, 1.05, num, label, deck.cyan)
        if i < len(steps) - 1:
            line = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, deck.e(x + 1.28), deck.e(5.64), deck.e(1.38), deck.e(0.05))
            line.fill.solid()
            line.fill.fore_color.rgb = deck.line
            line.line.fill.background()
    deck.text_box(s, "Типовой пользовательский маршрут", 0.9, 4.42, 6.7, 0.35, size=17, color=deck.navy, bold=True)

    # 5
    s = deck.slide()
    deck.title(s, "Архитектура системы", "Приложения Django связаны service-layer, а не дублированием формул в шаблонах", 5)
    layers = [
        ("UI и клиентский слой", "каталог, карточки, CAD-редактор, симулятор, лаборатория, обучение", deck.cyan),
        ("Django apps", "shop, accounts, orders, knowledge, Dolg_APP, административный контур", deck.blue),
        ("Service-layer", "engineering_lab, project_review, learning_grader, rule_ai, cad_import, formula_steps", deck.green),
        ("Expert-first core", "jsonschema, rule-engine, Pint, Lark, Z3, scikit-fuzzy, graph/formula services", deck.orange),
        ("Данные", "SQLite demo DB, товары, РЭБ-компоненты, схемы, уроки, попытки, review snapshots", deck.purple),
    ]
    ...
    deck.card(s, 1.35, 7.35, 13.2, 0.7, "Ключевой принцип", "Формулы, проверки, импорт, review и подсказки используются повторно в лаборатории, обучении, симуляторе и AI-помощнике.", deck.cyan, title_size=14, body_size=13)

    # 6
    s = deck.slide()
    deck.title(s, "Ключевые фрагменты реализации", "На слайд вынесены структуры, которые объясняют инженерный вывод", 6)
    code_font = "Consolas"
    blocks = [
        ("Expert finding", "rule_id: led_power_margin\nseverity: risk\nevidence: {power, limit, margin}\nrecommendation: increase resistor power rating", deck.blue),
        ("Unit-safe parsing", "parse('10 kOhm') -> 10000 Ohm\nparse('100 nF') -> 1e-7 F\nwarning: maybe 10 instead of 10k", deck.green),
        ("Constraint solver", "given Vin, Vf, Iled\nfind R in E24\ncheck power and tolerance\nreturn valid variants", deck.orange),
    ]
    for i, (title, code, color) in enumerate(blocks):
        ...
        deck.text_box(s, "Используется в review, обучении и подсказках.", x + 0.28, 5.45, 3.9, 0.45, size=12.7, color=deck.muted)
    deck.card(s, 1.1, 7.05, 13.6, 0.72, "Почему это важно", "Вывод системы можно объяснить: правило сработало по конкретным фактам, единицам измерения, ограничениям и данным проекта.", deck.cyan, title_size=14.2, body_size=13.2)

    # 7
    s = deck.slide()
    deck.title(s, "Реализованные модули", "Функции сгруппированы по инженерному маршруту пользователя", 7)
    cards = [
        ("Каталог", "89 товаров, 43 РЭБ-компонента, datasheet, категории, no-Wikimedia media-policy", deck.cyan),
    ...
    deck.card(s, 1.2, 7.0, 13.2, 0.78, "Итог по функциональности", "Сайт работает не только как магазин, а как связанный учебно-инженерный контур.", deck.cyan, title_size=14.2, body_size=13.5)
    ...
    deck.bullet_panel(s, 11.05, 1.45, 4.1, 1.95, "Что проверяется", ["наличие GND", "источник питания", "соединения", "номиналы"], deck.blue)
    deck.bullet_panel(s, 11.05, 3.8, 4.1, 1.65, "Что формируется", ["scheme_data", "BOM", "netlist", "review input"], deck.green)
    deck.bullet_panel(s, 11.05, 5.88, 4.1, 1.48, "Зачем это нужно", ["не просто рисунок", "данные для анализа"], deck.orange)

    # 9
    s = deck.slide()
    deck.title(s, "Симуляция и измерения", "Расчет, графики и метрики используются в лаборатории, review и обучении", 9)
    deck.image(s, sim, 0.75, 1.42, 9.15, 5.95, "Графики и результаты AC/DC/TRAN анализа")
    deck.card(s, 10.25, 1.45, 4.8, 1.58, "Режимы анализа", "DC / OP, AC, TRAN, Bode plot, FFT spectrum, Monte Carlo tolerance.", deck.blue, body_size=13.8)
    deck.card(s, 10.25, 3.33, 4.8, 1.58, "Метрики", "Напряжение узла, ток ветви, RMS, частота, duty cycle, мощность, температура.", deck.green, body_size=13.8)
    deck.card(s, 10.25, 5.21, 4.8, 1.58, "Инженерная оценка", "Норма, риск, перегрев, нужен запас. Результат не остается просто числом.", deck.orange, body_size=13.8)
    deck.text_box(s, "Expected vs measured связывает лабораторный расчет и фактическое измерение из симуляции.", 1.0, 7.55, 13.9, 0.35, size=14, color=deck.cyan, bold=True, align="center")

    # 10
    s = deck.slide()
    deck.title(s, "Проверка результата", "Тесты и демонстрационные проверки подтверждают готовность к защите", 10)
    ...
    for i, (value, label, color) in enumerate([("OK", "demo-ready", deck.green), ("OK", "integrity", deck.green), ("0", "warnings", deck.blue), ("89", "products", deck.cyan)]):
        deck.metric(s, 8.45 + i * 1.55, 5.92, 1.22, 1.0, value, label, color)
    deck.card(s, 8.25, 7.2, 6.65, 0.68, "Вывод", "Проверяется не только Django-конфигурация, но и наполнение, demo-сценарии, обучение, expert stack и media-policy.", deck.cyan, title_size=13.8, body_size=12.8)
    ...
    for i, (num, title, body) in enumerate(roadmap):
        ...
        deck.card(s, 2.05, y, 12.75, 0.75, title, body, [deck.blue, deck.green, deck.orange, deck.purple, deck.red][i], title_size=14.4, body_size=12.6)
    deck.card(s, 1.15, 7.35, 13.4, 0.68, "Принцип развития", "Сначала объяснимые экспертные системы и ограничения, затем оптимизация, и только после этого нейронные подсказки.", deck.cyan, title_size=13.8, body_size=13)

    # 12
    s = deck.slide(12)
    deck.text_box(s, "Спасибо за внимание!", 0.9, 1.02, 8.2, 0.75, size=36, color=deck.navy, bold=True)
    deck.text_box(s, "DOLG объединяет каталог, CAD/SIM, инженерную лабораторию, обучение и экспертную проверку проекта.", 0.94, 2.15, 7.45, 0.95, size=18, color=deck.text)
    deck.card(s, 1.0, 4.15, 5.9, 1.65, "Контактная информация", f"{AUTHOR}\nEmail: buryako@internet.com\nТелефон: +7 (903) 439-44-87", deck.cyan, title_size=15, body_size=13.4)
    deck.card(s, 7.55, 4.15, 6.9, 1.65, "Материалы к защите", "Диплом, речь и презентация актуализированы. Проверки проекта проходят, текущие данные зафиксированы в документах.", deck.green, title_size=15, body_size=13.4)
    ...
    download_white = DOWNLOADS / "Razrabotka-veb-prilozheniya-dlya-prodazhi-radio-i-elektronnyh-komponentov-so-vstroennymi-instrumenta_fixed_white_20260519.pptx"
...
def main() -> None:
    ...
    download_pptx = DOWNLOADS / "Razrabotka-veb-prilozheniya-dlya-prodazhi-radio-i-elektronnyh-компонентов-so-vstroennymi-instrumenta_updated_20260518.pptx"
