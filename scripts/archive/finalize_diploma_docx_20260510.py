...
def find_latest_input() -> Path:
    candidates = sorted(
        DOCS.glob('*010109*актуализировано.docx'), key=lambda p: p.stat().st_mtime, reverse=True
    )
    if not candidates:
...
def insert_table_after(
    anchor: Paragraph, caption: str, headers: list[str], rows: list[list[str]], size: int = 10
) -> Paragraph:
...
def insert_picture_after(
    anchor: Paragraph, image_path: Path, caption: str, width_inches: float = 5.9
) -> Paragraph:
...
def rebuild_appendices(doc: Document) -> None:
    ...
    anchor = insert_paragraph_after(
        anchor, 'Приложение А. Сводные сведения о реализации проекта', 'Heading 2'
    )
    anchor = insert_table_after(
        ...
        [
            [
                'shop',
                'Каталог, полки категорий, карточки товаров, корзина, BOM, сравнение',
                'shop/models.py; shop/views.py; shop/templates/shop/includes/product_card.html',
            ],
            ['accounts', 'Профиль, регистрация, адреса', 'accounts/models.py; accounts/views.py'],
            [
                'orders',
                'Заказы, повтор заказа, платежные сценарии',
                'orders/models.py; orders/views.py; orders/payment_views.py',
            ],
            [
                'knowledge',
                'Энциклопедия, статьи, внутренние ссылки и вложенные материалы',
                'knowledge/models.py; knowledge/views.py; knowledge/templates/knowledge/*',
            ],
            [
                'Dolg_APP',
                'Схемы, CAD, симуляция, AI, 3D, sharing',
                'Dolg_APP/models.py; Dolg_APP/views.py; templates/tools/*',
            ],
        ],
    )
    anchor = insert_table_after(
    ...
    anchor = insert_table_after(
        ...
        [
            [
                'Связь редактора схем с каталогом',
                'Сжато до архитектурного описания BOM, SKU-привязки и поиска компонентов',
                '2.5, приложение А',
            ],
            [
                'Серверный XLSX-экспорт BOM',
                'Оставлен как доказательство связки проектирования и закупки',
                '2.5, приложение Г',
            ],
            [
                'Browser-smoke, DRC/BOM и аудит UI',
                'Описание сокращено до таблицы контроля качества',
                '2.6, приложение Е',
            ],
            [
                'CAD, проекты и normalizeSchemeData',
                'Включено в описание устойчивого формата JSON и CAD-редактора',
                '2.2, 2.5',
            ],
            [
                'DC/AC/TRAN, SVG/PDF и netlist builder',
                'Сведено в раздел симуляции и кодовые листинги',
                '2.6, приложение Д',
            ],
            [
                'Приоритет ремонта CAD и симуляции',
                'Вынесено в план развития без дневниковых формулировок',
                'Заключение, приложение Е',
            ],
        ],
    )
    anchor = insert_table_after(
        ...
        [
            [
                'Ассортимент',
                'Каталог расширен до 89 товаров, включая 43 РЭБ-компонента; у всех позиций заполнены фотографии',
                'Карточки выглядят как реальные товарные позиции, а не как набор заглушек',
            ],
            [
                'Главная страница',
                'Вместо неоднозначного блока «Все товары» добавлены полки категорий и блок «Новые и актуальные позиции»',
                'Каталог читается как marketplace и масштабируется за пределы первой страницы',
            ],
            [
                'Карточки товаров',
                'Общий шаблон карточки используется в каталоге, похожих товарах и недавно просмотренных позициях',
                'Единый внешний вид снижает визуальные расхождения между разделами',
            ],
            [
                'Энциклопедия',
                'Добавлены внутренние ссылки и модель ArticleMaterial для изображений, gif-анимаций, видео, файлов и внешних материалов',
                'Статьи стали не только текстовыми, но и учебно-демонстрационными',
            ],
            [
                'Наполнение',
                'Опубликована 21 статья по 6 категориям и 22 дополнительных материала',
                'На защите можно показать предметное содержание по каждой категории',
            ],
        ],
    )
    ...
    images = [
        ...
        (
            ...
            'Рисунок Б.3 – Карточка товара с фотографией и техническими параметрами',
            13.2,
        ),
        (
            '07_product_related_cards.png',
            'Рисунок Б.4 – Блок похожих товаров с единым шаблоном карточек',
            14.1,
        ),
        (
            '08_knowledge_article_materials.png',
            'Рисунок Б.5 – Статья энциклопедии с внутренними ссылками и вложенными материалами',
            13.2,
        ),
...
if len(stable_text) >= PROMPT_CACHE_MIN_CHARS:
    ...
    anchor = insert_paragraph_after(
        anchor, 'Листинг В.2 – Преобразование сигнала лаборатории в SPICE-источник', 'Normal'
    )
    anchor.paragraph_format.first_line_indent = Cm(0)
    anchor = insert_code_after(
        doc,
...
if (!material.userData || !material.userData._shared) {
    ...
    anchor = insert_table_after(
        ...
        [
            ...
            [
                'Django tests',
                'около 130 тестов; browser-smoke пропускается без RUN_BROWSER_E2E',
                'Регрессия backend и доменной логики',
            ],
            [
                'Browser e2e',
                '16/16 OK при отдельном запуске',
                'Проверка canvas, CAD, модальных окон и графиков',
            ],
            [
                'AI+share suite',
                '31/31 OK в FAST_TESTS-режиме',
                'Проверка AI endpoint, sharing и catalog cache',
            ],
    ...
    anchor = insert_table_after(
        ...
        [
            [
                '1',
                'Ремонт и модернизация симуляции',
                'Стабильные расчеты, probes, курсоры, сохранение результатов',
            ],
