...
def update_speech_note() -> None:
    ...
    for docx_path in sorted(
        DOCS.glob('Речь_и_вопросы_к_защите_DOLG_*.docx'), key=lambda p: p.stat().st_mtime, reverse=True
    ):
...
class FinalDeck:
    ...
    def card(
        ...
        *,
        ...
        self.textbox(
            slide, title, x + 0.22, y + 0.2, w - 0.44, 0.35, size=title_size, bold=True, color=self.navy
        )
    ...
    def metric(
        ...
        self.panel(slide, x, y, w, h, accent)
        self.textbox(
            slide, value, x + 0.04, y + 0.17, w - 0.08, 0.38, size=22, bold=True, color=accent, align='center'
        )
        self.textbox(
            slide, label, x + 0.08, y + 0.65, w - 0.16, 0.36, size=13, color=self.muted, align='center'
        )
    ...
    def image(self, slide, path: Path, x: float, y: float, w: float, h: float, caption: str = '') -> None:
        ...
        slide.shapes.add_picture(
            str(path), self.e(x + (w - pw) / 2), self.e(y + (h - ph) / 2), width=self.e(pw), height=self.e(ph)
        )
        if caption:
            self.textbox(
                slide,
...
def build() -> Path:
    ...
    deck.card(
        ...
        2.35,
        'Проблема',
        'Каталог, datasheet, CAD, SPICE, BOM и заказ часто существуют отдельно. Номиналы, модели и результаты приходится переносить вручную.',
        deck.blue,
        body_size=14.6,
    )
    deck.card(
        ...
        2.35,
        'Цель',
        'Разработать веб-приложение для подбора и покупки компонентов со встроенными инструментами проектирования и симуляции схем.',
        deck.orange,
        body_size=14.6,
    )
    deck.card(
        ...
        2.35,
        'Задачи',
        'Реализовать каталог, учетные записи, корзину и заказы, редактор схем, SPICE-симуляцию, лабораторию, обучение и проверки.',
        deck.green,
        body_size=14.6,
    )
    for i, (value, label, color) in enumerate(
        [
            ('89', 'товаров', deck.cyan),
            ('43', 'РЭБ-компонента', deck.blue),
            ('5', 'расчетов', deck.green),
            ('29', 'заданий', deck.orange),
        ]
    ):
    ...
    deck.card(
        ...
        2.0,
        'Маркетплейсы',
        'Сильны в поиске, цене и заказе, но не проверяют схему и инженерные расчеты.',
        deck.blue,
    )
    deck.card(
        s,
        ...
        2.0,
        'Симуляторы',
        'Позволяют быстро экспериментировать, но обычно слабо связаны с реальными товарами.',
        deck.green,
    )
    deck.card(
        s,
        ...
        2.0,
        'CAD/EDA',
        'KiCad и Altium дают полный workflow, но имеют высокий порог входа.',
        deck.orange,
    )
    deck.card(
        s,
        ...
        2.0,
        'AI-CAD',
        'Дают подсказки, но требуют объяснимого источника инженерного вывода.',
        deck.purple,
    )
    deck.card(
        s,
        ...
        1.25,
        'Позиция DOLG',
        'Учебно-инженерный веб-сервис: каталог, схема, расчет, измерение, обучение и заказ находятся в одном контуре.',
        deck.cyan,
        title_size=17,
        body_size=15.0,
    )
    deck.fact(
        ...
        13.65,
        0.72,
        'Отличие проекта не в попытке заменить KiCad или Altium, а в связке: реальный компонент -> схема -> расчет -> проверка -> учебное объяснение.',
        deck.green,
    )
    ...
    deck.card(
        ...
        2.35,
        'Студенты',
        'Осваивают схемотехнику, решают практические задания и получают обратную связь по ошибкам.',
        deck.blue,
        body_size=15,
    )
    deck.card(
        ...
        2.35,
        'Радиолюбители',
        'Подбирают компоненты, проверяют простые узлы и формируют набор деталей для сборки.',
        deck.green,
        body_size=15,
    )
    deck.card(
        ...
        2.35,
        'Инженеры',
        'Сравнивают номиналы, оценивают запас, повторяемость решений и риски проекта.',
        deck.orange,
        body_size=15,
    )
    deck.bullet_card(
        ...
        11.9,
        1.65,
        'Что объединяет аудитории',
        [
            'нужны реальные компоненты, а не абстрактные элементы',
            'важны проверяемые расчеты и понятные ошибки',
            'полезна связь между схемой, обучением и BOM',
        ],
        deck.cyan,
        title_size=16,
        body_size=14.2,
    ...
    deck.fact(
        ...
        14.2,
        0.62,
        'Новые библиотеки не тянутся в шаблоны: они подключены через сервисы, поэтому один расчет используется в лаборатории, обучении, review и AI-помощнике.',
        deck.cyan,
    )

    s = deck.slide(6)
    deck.header(
        s, 'Каталог и наполнение', 'Товары связаны с документацией, статьями и инженерными сценариями', 6
    )
    ...
    deck.bullet_card(
        ...
        'Медиа-политика',
        [
            'нелепые фото заменяются',
            'Wikimedia больше не основной источник',
            'приоритет - производитель и техпорталы',
        ],
        deck.purple,
        body_size=13.5,
    )
    deck.fact(
        ...
        4.65,
        0.72,
        'Карточка товара стала входной точкой для datasheet, статьи, схемы, BOM и заказа.',
        deck.cyan,
    )
    ...
    deck.bullet_card(
        s, 11.15, 1.55, 3.95, 1.65, 'Проверки', ['GND', 'источник', 'соединения', 'номиналы'], deck.blue
    )
    deck.bullet_card(
        s,
        11.15,
    ...
    deck.bullet_card(
        s, 11.15, 5.68, 3.95, 1.35, 'Смысл', ['схема не картинка', 'это модель проекта'], deck.orange
    )
    deck.fact(
        s,
        0.78,
        ...
        14.32,
        0.56,
        'Одна и та же схема используется для DRC/ERC, генерации netlist, учебной проверки и будущего CAD-импорта.',
        deck.blue,
    )
    ...
    deck.card(
        ...
        1.45,
        'Измерения',
        'Напряжение узла, ток ветви, RMS, частота, duty cycle, мощность.',
        deck.green,
        body_size=13.8,
    )
    deck.card(
        ...
        1.45,
        'Оценка',
        'Норма, риск, перегрев, нужен запас. Результат объясняется.',
        deck.orange,
        body_size=13.8,
    )
    deck.fact(
        ...
        14.32,
        0.56,
        'Симуляция используется не как отдельный график, а как проверка гипотезы: расчетное значение сравнивается с измеренным.',
        deck.green,
    )
    ...
    deck.bullet_card(
        ...
        2.0,
        'Расчеты',
        ['транзисторный ключ', 'NE555', 'стабилизатор', 'тепловой запас', 'RC-антидребезг'],
        deck.green,
        body_size=14.2,
    )
    deck.bullet_card(
        ...
        1.85,
        'Использование',
        ['лаборатория', 'обучающие задания', 'Engineering Review'],
        deck.cyan,
        body_size=14.2,
    )
    deck.fact(
        ...
        4.85,
        0.82,
        'Лаборатория возвращает не только число, но и инженерную оценку: норма, риск, перегрев или нужен запас.',
        deck.orange,
    )
    ...
    deck.card(
        ...
        2.05,
        'Практикум',
        '4 маршрута, 13 уроков, 29 заданий: численные ответы, сборка схемы, измерение результата.',
        deck.blue,
        body_size=14.3,
    )
    deck.card(
        ...
        2.05,
        'Review',
        'Design Health Score, DRC/ERC, BOM-риски, наличие GND, источника, моделей и запаса.',
        deck.green,
        body_size=14.3,
    )
    deck.card(
        ...
        2.05,
        'AI-помощник',
        'Самописный rule_ai отвечает по фактам проекта, правилам, расчетам, статьям и заданиям.',
        deck.orange,
        body_size=14.3,
    )
    deck.bullet_card(
        ...
        'Expert-first подход',
        [
            'сначала объяснимые правила и факты',
            'затем constraint/optimization',
            'нейронная подсказка только поверх проверяемого baseline',
        ],
        deck.cyan,
        title_size=16.5,
        body_size=14.5,
    ...
    deck.fact(
        ...
        13.25,
        0.62,
        'Учебное задание может строиться из реальной ошибки проекта: нет GND, неверный номинал, перегрев или расхождение расчет/симуляция.',
        deck.purple,
    )
    ...
    checks = [
        ...
        ('demo-ready', 'OK'),
        ('data-integrity', 'OK'),
        ('targeted tests', 'expert / learning / search'),
    ]
    for i, (title, body) in enumerate(checks):
        deck.card(
            ...
            title,
            body,
            [deck.green, deck.cyan, deck.blue, deck.orange][i],
            title_size=15,
            body_size=14.2,
        )
    deck.bullet_card(
    ...
    deck.bullet_card(
        ...
        2.05,
        'Поздний этап',
        ['OR-Tools для BOM-оптимизации', 'RDFLib для онтологии компонентов', 'PyTorch/GOLEM для deep hints'],
        deck.purple,
        title_size=16.5,
        body_size=14.4,
    )
    ...
    deck.card(
        ...
        1.55,
        'Материалы',
        'Диплом, речь и презентация актуализированы. Проверки проекта проходят.',
        deck.green,
        title_size=15.5,
        body_size=13.8,
    )
    ...
    download_final = (
        DOWNLOADS
        / 'Razrabotka-veb-prilozheniya-dlya-prodazhi-radio-i-elektronnyh-komponentov-so-vstroennymi-instrumenta_final_clean_20260519.pptx'
    )
