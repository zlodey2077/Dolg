...
def update_diploma() -> None:
    ...
    if len(doc.tables) >= 13:
        ...
        table.cell(
            ...
            'POST /api/ai/chat/, self-hosted rule_ai, Expert trace из review, demo-mode без внешнего ключа'
        )
        table.cell(
            4, 2
        ).text = 'Ответ строится по данным схемы, BOM, review, expert findings и статьям; внешняя LLM не является обязательной'

        table = doc.tables[5]
        rows = [
            (
                'Expert Review',
                'Design Health Score, DRC/ERC, BOM-риск, derating',
                'ProjectReview, expert_rules, PDF/HTML отчет',
            ),
            (
                'Measurement Core',
                'Ожидаемое против измеренного, сохранение метрик',
                'ProjectMeasurement, simulation_analysis, lab_measurements',
            ),
            (
                'CAD Import',
                'LTspice/SPICE/KiCad subset в scheme_data',
                'Lark parser, import preview, review после импорта',
            ),
            (
                'Expert Assistant',
                'Объяснение ошибок и план исправления',
                'rule_ai, Expert trace, graph metrics, BOM facts',
            ),
            (
                'Neural deep analysis',
                'Вероятностные подсказки после expert baseline',
                'PyTorch/GOLEM как отдельный будущий sprint',
            ),
        ]
        ...
        table.cell(
            ...
            'Опубликованы 21 статья, 50 дополнительных материалов, 4 учебных маршрута, 13 уроков и 29 заданий'
        )
        table.cell(
            5, 2
        ).text = 'На защите можно показать связку энциклопедии, лаборатории, практикума и review'
        ...
        rows = [
            ...
            (
                ...
                'OK, включая expert_stack',
                'Проверка URL, learning data и библиотечных smoke-сценариев',
            ),
            (
                'check_data_integrity --json',
                'OK, 0 errors, 0 warnings',
                'Аудит каталога, media, статей, ссылок и demo-схем',
            ),
            (
                'Targeted regression',
                '18/18 expert, 16/16 learning, 8/8 search',
                'Проверка expert-first слоя и совместимости обучения',
            ),
        ]
        for row_idx, values in enumerate(rows, start=1):
            for col_idx, value in enumerate(values):
        ...
        rows = [
            (
                '1',
                'Measurement Core',
                'Probes, сохранение измерений, expected vs measured, связь лаборатории и симулятора',
            ),
            ...
            (
                ...
                'Комментарии к товарам, урокам, статьям, demo-схемам и ProjectReview',
            ),
    ...
    if not any('Приложение Е. Актуализация реализации от 19.05.2026' in p.text for p in doc.paragraphs):
        ...
        rows = [
            (
                'Expert rules',
                'jsonschema + rule-engine, default_rules.json',
                'ProjectReview, rule_ai, demo-ready',
            ),
            ('Unit-safe parsing', 'Pint, engineering_units.py', 'Лаборатория, learning grader, review'),
            (
                'Constraint solving',
                'z3-solver, constraint_solver.py',
                'Подбор LED, делителя, RC, NE555, стабилизатора',
            ),
            ('CAD parsing', 'Lark, cad_parsers.py', 'SPICE/LTspice subset перед нормализацией в scheme_data'),
            (
                'Risk scoring',
                'scikit-fuzzy, risk_scoring.py',
                'Мягкая оценка риска проекта поверх экспертных фактов',
            ),
        ]
        for values in rows:
...
class DarkDeck:
    ...
    def text_box(
        self, slide, text, x, y, w, h, *, size=12, color=None, bold=False, align='left', font='Segoe UI'
    ):
    ...
    def card(
        self, slide, x, y, w, h, title='', body='', accent=None, *, title_size=13, body_size=9.7, fill=None
    ):
        ...
        if title:
            self.text_box(
                slide, title, x + 0.18, y + 0.16, w - 0.3, 0.34, size=title_size, color=self.title, bold=True
            )
        if body:
    ...
    def metric(self, slide, x, y, w, h, value, label, color=None):
        ...
        self.text_box(
            slide, label, x + 0.04, y + 0.48, w - 0.08, 0.26, size=7.8, color=self.muted, align='center'
        )
...
def build_presentation() -> None:
    ...
    deck.text_box(
        s,
        'каталог → расчёт → схема → измерение → review → BOM → заказ',
        7.35,
        7.78,
        7.55,
        0.28,
    ...
    deck.title_block(
        s,
        'Актуальность и цель',
        'Компонент, схема, расчёт и заказ должны жить в одном инженерном маршруте',
        2,
    )
    deck.card(
        s,
        ...
        2.35,
        'Разрыв инструментов',
        'Каталог, datasheet, CAD, SPICE, BOM и заказ часто работают отдельно. Из-за ручного переноса теряются номиналы, модели, документация и результаты измерений.',
        deck.blue,
        body_size=10.6,
    )
    deck.card(
        ...
        'Учебным и малым инженерным проектам нужен быстрый web-путь: подобрать компонент, проверить схему, получить оценку риска и сформировать заказ.',
        deck.orange,
        body_size=10.6,
    )
    deck.card(
        ...
        2.35,
        'Цель работы',
        'Разработать DOLG для подбора, покупки, проектирования, симуляции, обучения и инженерной проверки результата в едином интерфейсе.',
        deck.green,
        body_size=10.6,
    )
    for idx, data in enumerate(
        ...
        deck.metric(
            ...
            data[0],
            data[1],
            [deck.blue, deck.cyan, deck.green, deck.purple, deck.orange, deck.red, deck.cyan][idx],
        )
    deck.card(
        s,
        1.05,
        ...
        'DOLG показывает связку электронной коммерции и инженерной проверки: пользователь выбирает реальный компонент, применяет его в схеме, проверяет расчетом/симуляцией и получает BOM для заказа.',
        deck.cyan,
        title_size=12,
        body_size=10.2,
    )
    ...
    s = deck.slide()
    deck.title_block(
        s, 'Анализ решений', 'Позиционирование относительно магазинов, онлайн-симуляторов и CAD/EDA', 3
    )
    deck.table(
        s,
        0.75,
        ...
        14.45,
        4.25,
        ['Класс', 'Сильная сторона', 'Ограничение', 'DOLG'],
        [
            (
                'Интернет-магазины',
                'каталог, цена, наличие, заказ',
                'нет проверки схемы',
                'BOM и товарная карточка связаны со схемой',
            ),
            (
                'Онлайн-симуляторы',
                'быстрые учебные эксперименты',
                'абстрактные элементы',
                'схема работает рядом с реальными позициями',
            ),
            (
                'CAD / EDA',
                'полный цикл разработки',
                'порог входа и отдельная закупка',
                'упрощенный web-маршрут для учебных задач',
            ),
            (
                'AI-CAD сервисы',
                'подсказки поверх схемы',
                'риск непрозрачных выводов',
                'expert-first trace перед нейронным анализом',
            ),
            (
                'DOLG',
                'каталог + CAD/SIM + lab + learning + review',
                'демонстрационный масштаб ВКР',
                'единый маршрут от компонента до заказа',
            ),
        ],
    ...
    deck.card(
        ...
        1.15,
        'Практическая ниша',
        'Проект не конкурирует с промышленными PCB CAD напрямую. Он закрывает сценарий, который редко собран в одном месте: компонент → расчёт → схема → измерение → обучение → review → BOM → заказ.',
        deck.cyan,
        title_size=12.5,
        body_size=10.8,
    )
    ...
    s = deck.slide()
    deck.title_block(
        s, 'Целевая аудитория и сценарии', 'Три группы пользователей и единый рабочий маршрут', 4
    )
    deck.card(
        s,
        0.75,
        ...
        1.75,
        'Студенты',
        'Изучают схемотехнику через расчёты, готовые схемы, практикум и автопроверку. Важны подсказки и обратная связь.',
        deck.blue,
        body_size=10,
    )
    deck.card(
        ...
        1.75,
        'Радиолюбители',
        'Подбирают компоненты под идею, быстро проверяют схему и собирают набор деталей для покупки.',
        deck.green,
        body_size=10,
    )
    deck.card(
        ...
        1.75,
        'Инженеры',
        'Сравнивают номиналы, оценивают запас, сохраняют проверенные схемы и экспортируют BOM.',
        deck.orange,
        body_size=10,
    )
    deck.card(
        ...
        3.1,
        'Типовой маршрут',
        '1. Найти компонент → 2. Открыть карточку и datasheet → 3. Рассчитать узел в лаборатории → 4. Собрать схему → 5. Запустить DC/AC/TRAN → 6. Сравнить ожидаемое и измеренное → 7. Получить review → 8. Сформировать BOM и заказ.',
        deck.cyan,
        title_size=15,
        body_size=13,
    )
    for idx, data in enumerate(
        [('21', 'статья'), ('50', 'материалов'), ('4', 'tracks'), ('13', 'lessons'), ('29', 'tasks')]
    ):
        deck.metric(
            s,
            5.75 + idx * 1.82,
            ...
            data[0],
            data[1],
            [deck.blue, deck.green, deck.purple, deck.orange, deck.red][idx],
        )
    deck.text_box(
        s,
        'Маршрут на слайде показывает, что обучение, лаборатория и review не отдельные витрины, а продолжение работы с компонентом.',
        5.8,
        6.75,
        8.75,
        0.48,
    ...
    # 5
    s = deck.slide()
    deck.title_block(s, 'Архитектура системы', 'Django apps + service-layer + expert-first контур', 5)
    deck.card(
        s,
        0.65,
        1.35,
    ...
    deck.card(
        ...
        0.8,
        'Принцип развития',
        'Крупная фича добавляется вместе с обучающим блоком и документацией; формулы, проверки и метрики остаются в Python service-layer.',
        deck.cyan,
        title_size=12,
        body_size=10,
    )
    ...
    deck.title_block(
        ...
        'Кодовые структуры, через которые объясняется инженерная логика проекта',
        6,
    )
    ...
    deck.text_box(
        s,
        'Review и AI получают не просто строку ошибки, а проверяемый вывод: правило, факты, уверенность и рекомендацию.',
        1.0,
        5.35,
        4.0,
        0.82,
    ...
    deck.text_box(
        s,
        'Pint снимает конфликт единиц между лабораторией, review и учебными задачами.',
        5.92,
        5.35,
        4.0,
        0.82,
    ...
    deck.text_box(
        s,
        'Z3 подбирает варианты при ограничениях; финальный инженерный verdict остаётся у rule/review слоя.',
        10.85,
        5.35,
        4.0,
        0.82,
    ...
    s = deck.slide()
    deck.title_block(
        s, 'Реализованные модули', 'Функции проекта поверх общего каталога, данных и сервисного слоя', 7
    )
    modules = [
        (
            'Каталог и BOM',
            '89 товаров, 43 РЭБ, фильтры, datasheet, сравнение, XLSX/CSV, add-all-to-cart.',
            deck.blue,
        ),
        (
            'Инженерная лаборатория',
            'Транзисторный ключ, NE555, стабилизатор, RC-антидребезг, тепловой запас.',
            deck.green,
        ),
        (
            'Практикум',
            '4 маршрута, 13 уроков, 29 заданий: math_numeric, circuit_build, simulation_measure.',
            deck.purple,
        ),
        ('CAD/SIM', 'Canvas2D редактор, УГО, DRC, ngspice.wasm, DC/AC/TRAN и Pro-аналитика.', deck.orange),
        (
            'Engineering Review',
            'Design Health Score, DRC/ERC, BOM-риск, derating, expert findings, PDF/HTML.',
            deck.cyan,
        ),
        (
            'Контроль качества',
            'check, migrations, demo-ready, data-integrity, no-Wikimedia и targeted tests.',
            deck.red,
        ),
    ]
    ...
    deck.card(
        ...
        0.76,
        'Важно',
        'Ассортимент и единые карточки являются свойствами каталога, а функции — это инженерные сценарии поверх данных: lab, CAD/SIM, learning, review, BOM и order flow.',
        deck.cyan,
        title_size=12,
        body_size=9.8,
    )
    ...
    s = deck.slide()
    deck.title_block(
        s, 'CAD-редактор', 'Отдельный режим для чертежей, УГО, ГОСТ-шаблонов и передачи схемы в симулятор', 8
    )
    ...
    deck.card(
        ...
        1.35,
        'Режим CAD',
        'Canvas2D, сетка, snap-привязка, слои, аккуратное позиционирование.',
        deck.blue,
    )
    deck.card(
        s,
        ...
        1.35,
        'ГОСТ и чертежи',
        'Рамки, титульные блоки, размерные линии и элементы ЕСКД-логики.',
        deck.green,
    )
    deck.card(
        s,
        ...
        1.35,
        'Библиотека УГО',
        'Символы компонентов, узлы, подписи и подготовка схемы к расчету.',
        deck.orange,
    )
    deck.card(
        s,
        ...
        1.35,
        'Экспорт',
        'PDF, PNG, SVG, JSON; JSON идет в симуляцию, review и demo-сценарии.',
        deck.purple,
    )
    ...
    s = deck.slide()
    deck.title_block(
        s, 'Симуляция и измерения', 'Схема превращается в расчёт, графики, метрики и инженерную оценку', 9
    )
    deck.card(
        s,
        0.75,
    ...
    deck.card(
        ...
        1.55,
        'Pro-аналитика',
        'FFT spectrum, Bode plot, Monte Carlo tolerance, THD/SINAD/ENOB, parameter sweep, server-side fallback.',
        deck.green,
        body_size=9.4,
    )
    deck.card(
        ...
        1.62,
        'Метрики',
        'Напряжение узла, ток ветви, RMS, частота, duty cycle, мощность элемента и температура.',
        deck.orange,
        body_size=9.6,
    )
    deck.card(
        ...
        0.78,
        'Связь с обучением',
        'Expected vs measured используется в лаборатории, review и simulation_measure задачах.',
        deck.cyan,
        title_size=11.4,
        body_size=8.8,
    )
    ...
    s = deck.slide()
    deck.title_block(
        s, 'Проверка результата', 'Тесты, demo-ready, data-integrity и лабораторный результат', 10
    )
    deck.table(
        s,
        0.75,
    ...
    deck.card(
        ...
        1.2,
        'Что подтверждается',
        'Каталог, media-policy, статьи, demo-схемы, learning, expert stack и лабораторные расчеты проверяются отдельными smoke/regression сценариями.',
        deck.cyan,
        title_size=11.8,
        body_size=9.3,
    )
    ...
    # 11
    s = deck.slide()
    deck.title_block(s, 'План развития', 'Пакетный подход: фича + обучалка + документация', 11)
    road = [
        (
            '1. Expert Review Core',
            'расширение rule packs, explainable findings, Learning-by-review',
            deck.blue,
        ),
    ...
    deck.card(
        ...
        '1. Экспертная система объясняет вывод через правила, факты, формулы, граф схемы, измерения и BOM.\n\n2. Constraint/optimization подбирает допустимые варианты номиналов и BOM.\n\n3. Нейронный слой появляется только как поздний анализ и не выносит финальный инженерный verdict.',
        deck.cyan,
        title_size=15,
        body_size=12.7,
    )
    deck.text_box(
        s,
        'Такой порядок снижает риск непрозрачного AI и делает выводы пригодными для защиты.',
        7.7,
        6.95,
        6.7,
        0.4,
    ...
    deck.text_box(
        s,
        'DOLG объединяет каталог, CAD/SIM, инженерную лабораторию, практикум и экспертную проверку проекта в одном тёмном рабочем интерфейсе.',
        0.9,
        2.0,
        7.4,
        0.95,
    ...
    deck.card(
        ...
        2.05,
        'К защите готово',
        'Диплом, речь и презентация актуализированы. Demo-ready и data-integrity проходят; expert-first stack зафиксирован в документах.',
        deck.green,
        title_size=14,
        body_size=12.2,
    )
    ...
    download_fixed = (
        DOWNLOADS
        / 'Razrabotka-veb-prilozheniya-dlya-prodazhi-radio-i-elektronnyh-komponentov-so-vstroennymi-instrumenta_fixed_dark_20260519.pptx'
    )
