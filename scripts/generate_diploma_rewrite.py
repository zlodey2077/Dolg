...
def build_document(facts: Facts) -> Document:
    ...
    add_table(
        ...
        [
            ["Гость", "Просмотр каталога, базы знаний, демо-схем и симулятора", "Понимание возможностей системы без регистрации"],
            ["Пользователь", "Покупка товаров, создание проектов, сохранение версий и запусков симуляции", "Персональный инженерный рабочий контур"],
            ["Менеджер", "Работа с заказами, товарами и содержанием каталога", "Контроль коммерческой части проекта"],
            ["Администратор", "Управление пользователями, справочниками, правами и данными", "Полное сопровождение системы"],
        ],
    )
    ...
    add_table(
        ...
        [
            ["Каталог товаров, фильтры, поиск, карточка товара", "Реализовано", f"В базе {facts.products} товаров и {facts.categories} категорий."],
            ["Корзина и оформление заказа", "Реализовано", "Есть модели заказов, доставки, платежей и тесты orders."],
            ["Личный кабинет и адреса пользователя", "Реализовано", "Профиль создаётся автоматически, есть пользовательские тесты accounts."],
            ["Редактор принципиальных схем", "Реализовано частично", "Базовый редактор работает; продолжается ремонт UI, netlist builder и модульная декомпозиция."],
            ["CAD-раздел", "Реализовано частично", "Есть базовый save/load и ГОСТ-шаблоны; требуется модернизация под инженерные сценарии."],
            ["Симуляция DC/AC/TRAN", "Реализовано частично", "Есть ngspice.wasm и JS-MNA fallback; приоритет — обработка ошибок, probes, курсоры, сохранение графиков."],
            ["DRF, JWT, Redis, Celery, PostgreSQL", "Перспектива", "В текущем коде основа — Django MVT, SQLite и синхронные сценарии."],
            ["What if, thermal, AI, 3D, virtual lab", "Перспектива", "Функции включены в roadmap после стабилизации CAD и симулятора."],
        ],
    )
...
""",
    ...
    add_table(
        ...
        [
            ["shop", "Category, Product, Cart, Compare", "Каталог, параметры товаров, BOM-сопоставление и покупка"],
            ["accounts", "UserProfile, Address", "Профиль пользователя, контактные данные и адреса"],
            ["orders", "Order, OrderItem, Shipment, PaymentTransaction", "Оформление заказа, доставка и платежная история"],
            ["knowledge", "KnowledgeCategory, Article", "Справочная база по электронике и компонентам"],
            ["Dolg_APP", "SchematicProject, ProjectVersion, SimulationRun", "Проекты схем, история версий и результаты симуляций"],
        ],
    )
    add_table(
        document,
...
""",
    ...
    add_table(
        ...
        [
            ["Каталог и BOM", "Выполнено", "Связь схемы с товарами каталога, catalog_ref, CSV/XLSX BOM, add-all-to-cart"],
            ["Проекты и версии", "Выполнено", "SchematicProject, ProjectVersion, demo-проекты, save/load, история изменений"],
            ["Симуляция DC/AC/TRAN", "Выполнено частично", "SPICE netlist, ngspice.wasm, JS-MNA fallback, browser smoke; требуется расширить probes и edge-cases"],
            ["Отрисовка аналитики", "Выполнено частично", "Панель результатов перенесена под свойства компонента, добавлены проверки читаемости и overflow"],
            ["CAD-базис", "Выполнено частично", "ГОСТ-шаблон, save/load, первый desktop/mobile visual baseline, исправление мобильной высоты canvas"],
            ["Ремонт симулятора", "Ближайший этап", "Диагностика ngspice, курсоры графиков, сохранение расчётов, устойчивые AC/TRAN-сценарии"],
            ["Оптимизация УГО-схем", "Выполнено частично", "Переключатель modern-стиля убран; оставлен практичный УГО-режим, скрыта подсказка canvas, ускорена отрисовка больших схем"],
            ["Чертёжный шаг схем", "Выполнено", "Введён отдельный шаг хода, не связанный с визуальной сеткой; snap компонентов, изгибы и демо-схемы нормализованы по drawing_step=30"],
            ["Ремонт CAD", "Ближайший этап", "Screenshot-baseline регрессия, размеры, слои, штамп, привязки, горячие клавиши"],
            ["Killer-функции", "Перспектива", "What if-слайдер, тепловой анализ, AI-ассистент, 3D-просмотр, виртуальная лаборатория"],
        ],
    )
    add_paragraph(
        document,
...
""",
    ...
    add_table(
        ...
        [
            ["Каталог и карточки товаров", "Реализовано", "Фильтры, поиск, datasheet, warranty PDF, параметры товара"],
            ...
            ["Симуляция DC/AC/TRAN", "Реализовано частично", "E2E покрывает DC/AC/TRAN, пустой AC stdout и отказ обоих движков; нужны probes, курсоры и расширенные edge-cases ngspice"],
            ["Аналитика симуляции", "Реализовано частично", "Панель результатов перенесена под свойства компонента; desktop/mobile visual smoke проверяет читаемую ширину и отсутствие overflow"],
            ["CAD", "Реализовано частично", "Есть save/load, ГОСТ-шаблон и desktop/mobile visual-smoke; нужна screenshot-baseline регрессия и инженерные инструменты"],
            ["Экспорт схем", "Реализовано", "SVG/PDF browser-smoke проверяет скачивание и сигнатуры файлов"],
            ["Модалки и предупреждения симулятора", "Реализовано частично", "BOM/netlist/project picker и export warnings проверяются на desktop/mobile; нужен screenshot-baseline для сложных состояний"],
            ["Визуальный стиль схем", "Реализовано частично", "Оставлен единый УГО-режим; добавлены отдельный шаг хода, нормализация демо-схем и performance-smoke больших схем; требуется дальнейшая аккуратная перерисовка без потери практичности"],
            ["Роли и администрирование", "Реализовано частично", "Есть Django admin и группы; статистика менеджера остаётся в плане"],
        ],
    )
    ...
    add_table(
        ...
        [
            ["python manage.py check", "OK; при DEBUG disabled ожидаемо предупреждение о SECRET_KEY", "Проверка конфигурации Django"],
            ["scripts/run_checks.ps1", f"{facts.tests_total} обнаружено; {facts.tests_ok} OK; {facts.tests_skipped} skipped; coverage {facts.coverage}", "Основной тестовый контур"],
            ["scripts/run_browser_e2e.ps1", f"{facts.browser_ok} OK", "Playwright smoke для simulation, CAD, projects, экспортов, модалок и графиков"],
...
""",
    ...
    add_table(
        ...
        [
            ...
            ["Симулятор", "`/simulation/`", "DC/AC/TRAN, единый УГО-режим, отдельный шаг хода, нормализованные демо-схемы, ускоренная отрисовка больших схем, графики, аналитика под свойствами компонента, SVG/PDF, BOM/netlist-модалки, предупреждения, сохранение запусков"],
    ...
    add_table(
        ...
        [
            ["Связь схемы с каталогом", "catalog_ref, catalog_slug, параметры товара, BOM-сопоставление", "Карточка компонента, BOM-модалка, XLSX-файл"],
            ["Вынос JS-модулей", "scheme-normalizer.js, scheme-export.js, scheme-bom.js, scheme-netlist.js", "Фасады старых функций и новые browser fixtures"],
            ["DC/AC/TRAN", "fixtures для делителя, RC-фильтра и transient-графика", "Canvas графиков, SimulationRun, browser e2e"],
            ["Ошибки симуляции", "Диагностика пустого AC stdout и двойного отказа движков", "Сообщения в results panel вместо пустого графика"],
            ["Аналитика под свойствами", "Новая simulation-analysis-panel под component-properties", "Desktop/mobile smoke без горизонтального overflow"],
            ["CAD visual baseline", "Проверка ГОСТ-шаблона, панелей, canvas и мобильной высоты", "Тест, выявивший и подтвердивший исправление 150 px canvas"],
            ["Оптимизация рендера схем", "Скрыта canvas-подсказка, убран export из аналитики, оставлен УГО-режим, добавлен fast-path больших схем", "Targeted browser visual/performance smoke 3/3 OK"],
            ["Чертёжный шаг схем", "Отдельный шаг хода, snap/маршрутизация без привязки к визуальной сетке, нормализация 11 демо-схем", "Настройка в UI, drawing_step=30, targeted browser smoke 3/3 OK"],
        ],
    )
