...
Пользователь
  ...
  -> CartItem через /bom/add-all/
    ...
    _add_table(
        ...
        ["Проверка", "Результат"],
        [
            ["python manage.py test shop --settings=Dolg_PR.settings_test", "38 тестов, OK (после расширения DRC/BOM-проверок)"],
            ["Общий ожидаемый набор", "84 обнаруженных теста: accounts — 8, shop — 38, orders — 16, Dolg_APP — 21 + 1 optional browser-smoke"],
            ["Проверяемые сценарии", "Поиск товара компонента, NPN/PNP → transistors, явный catalog_ref в BOM"],
        ],
    )
...
Модальное окно BOM
  ...
  -> BOM_YYYY-MM-DD.xlsx
    ...
    _add_table(
        ...
        [
            ["python manage.py test shop --settings=Dolg_PR.settings_test", "38 тестов, OK (после расширения DRC/BOM-проверок)"],
            ["powershell -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "89 тестов обнаружено: 83 OK, 6 skipped; coverage 78 %"],
  ...
  -> /cart/
    ...
    _add_table(
        ...
        [
            ...
            ["python manage.py test shop Dolg_APP --settings=Dolg_PR.settings_test", "60 тестов обнаружено: 59 OK, 1 skipped"],
        ],
    )
    ...
    document.add_paragraph(
        "Контроль источников: в список источников добавлена официальная "
        "документация Playwright Python [42]."
    )
    return True
  ...
  -> normalizeSchemeData()
    ...
    _add_table(
        ...
        [
            ["powershell -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "89 тестов обнаружено: 83 OK, 6 skipped; coverage 78 %"],
            [".\\scripts\\run_browser_e2e.ps1", "6/6 browser-smoke OK"],
            ["Проверяемые browser-сценарии", "/simulation/ BOM/XLSX/cart; /cad/ save-load; /projects/ versions-runs-open"],
        ],
    )
  ...
  -> /projects/: create -> save-scheme -> versions -> save-simulation -> /simulation/
    ...
    _add_table(
        ...
        [
            ["powershell -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "89 тестов обнаружено: 83 OK, 6 skipped; coverage 78 %"],
            [".\\scripts\\run_browser_e2e.ps1", "6/6 browser-smoke OK"],
            ["Проверяемые browser-сценарии", "/simulation/ BOM/XLSX/cart; DC; SVG/PDF; /cad/ save-load; /projects/ versions-runs-open"],
        ],
    )
  ...
  -> browser fixtures
    ...
    _add_table(
        ...
        [
            ["powershell -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "89 тестов обнаружено: 83 OK, 6 skipped; coverage 78 %"],
            [".\\scripts\\run_browser_e2e.ps1", "6/6 browser-smoke OK"],
            ["Проверяемые browser-сценарии", "/simulation/ BOM/XLSX/cart; DC; SVG/PDF; netlist fixtures; /cad/; /projects/"],
        ],
    )
...
def append_scheme_netlist_elements_iteration(document: Document) -> bool:
    ...
    _add_table(
        ...
        [
            ...
            ["Switch", "`R<id>SW nA nB 1m` или комментарий об open-state", "`{ type: 'R', value: 1e-3 }` для замкнутого ключа"],
        ],
    )
...
def append_scheme_netlist_analysis_iteration(document: Document) -> bool:
    ...
    _add_table(
        ...
        [
            ["DC", "`.op`", "`dcLines == ['.op']`"],
            ["AC", "`.ac DEC 20 1 1Meg` + `.print ac vdb(N) vp(N)` по всем ненулевым узлам", "Проверяется `.print ac vdb(1) vp(1) vdb(2) vp(2)`"],
            ["TRAN/pulse", "`.tran <tStep> <tStop>` с `formatSpiceNumber()`", "Проверяется `.tran 2.5000e-4 0.25` при `simTime=250` мс"],
        ],
    )
...
buildSpiceNetlist() facade
  ...
  -> .end and legacy return contract
    ...
    _add_table(
        ...
        ["Команда", "Результат"],
        [
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_browser_e2e.ps1", "6/6 browser-smoke OK"],
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "89 тестов OK, 6 skipped; coverage 78 %"],
            [".venv\\Scripts\\python.exe -m py_compile scripts\\generate_dolg_report.py scripts\\update_diploma_progress.py", "OK"],
        ],
    )
...
def append_ac_browser_smoke_iteration(document: Document) -> bool:
    ...
    _add_table(
        ...
        [
            ["Схема RC-фильтра", "`applySchemeData()` создаёт V/R/C/GND и связи между ними", "Фиксирует типовой AC-сценарий из дипломной области"],
            ["Режим анализа", "`#analysis-type` переключается в `ac`; тест проверяет `msg.analysis == 'ac'`", "Защищает UI-контракт выбора режима"],
            ["Состав circuit", "Проверяется наличие `C` в `circuit.elements`", "Подтверждает, что netlist builder передал ёмкость в fallback-движок"],
            ["Графики", "Проверяются `#sim-graph-mag`, `#sim-graph-phase` и наличие не фоновых пикселей", "Защищает фактическую отрисовку АЧХ/ФЧХ, а не только текст"],
        ],
    )
...
const hasNonBackgroundPixel = (id) => {
    ...
    _add_table(
        ...
        ["Команда", "Результат"],
        [
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_browser_e2e.ps1", "7/7 browser-smoke OK"],
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "90 тестов OK, 7 skipped; coverage 78 %"],
        ],
    )
...
def append_tran_browser_smoke_iteration(document: Document) -> bool:
    ...
    _add_table(
        ...
        [
            ...
            ["Параметры времени", "`#sim-time = 50`; ожидается `tStop = 0.05`", "Параметры `.tran` и fallback синхронизированы"],
            ["Схема", "В `circuit.elements` присутствует `C`", "RC-цепь передана в движок"],
            ["График", "Проверяется `#sim-graph` и не фоновые пиксели", "Canvas временного графика реально отрисован"],
        ],
    )
...
Browser/e2e simulation coverage
  ...
  -> netlist fixtures: aliases, DRC, .op/.ac/.tran directives
    ...
    _add_table(
        ...
        ["Команда", "Результат"],
        [
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_browser_e2e.ps1", "8/8 browser-smoke OK"],
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "91 тест OK, 8 skipped; coverage 78 %"],
        ],
    )
...
def append_visual_layout_smoke_iteration(document: Document) -> bool:
    ...
    _add_table(
        ...
        [
            ["1366×900", "Загружен делитель напряжения, выполнен DC fallback", "Нет horizontal overflow; canvas > 700×250; видны панели компонентов, управления и результаты; screenshot создаётся"],
            ["390×844", "Загружен RC-фильтр, выполнен AC fallback", "Нет horizontal overflow; ключевые панели внутри viewport; canvas АЧХ содержит не фоновые пиксели"],
        ],
    )
...
return false;
    ...
    _add_table(
        ...
        ["Команда", "Результат"],
        [
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_browser_e2e.ps1", "10/10 browser-smoke OK"],
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "93 теста OK, 10 skipped; coverage 78 %"],
        ],
    )
...
page.wait_for_function(async (projectId) => {
    ...
    _add_table(
        ...
        [
            ...
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_browser_e2e.ps1", "11/11 browser-smoke OK"],
            ["powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\run_checks.ps1", "94 теста OK, 11 skipped; coverage 78 %"],
        ],
    )
